...
class application:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("Object Locked")
        self.root.bind("<Key>", self.zoomBox)
        self.rescale = 2
        self.widthScreen = 500
        self.heightScreen = 500
        self.boxSize = 50
        self.muchImg = 20
        self.boxObj = []
        self.listImgObj = []
        ...
        self.root.geometry(str(self.widthScreen) + "x" + str(self.heightScreen))
        self.layout()
        img = Image.open("tryOne.png")
        self.showImg(img)

        threading.Thread(target=self.makeAnim).start()

    def layout(self):
        self.imgFrame = Label(self.root, text="Loading")
        self.imgFrame.bind("<B1-Motion>", self.trackMouse)
        self.imgFrame.place(x=0, y=0)

        self.indexRec = Label(self.root)
        self.indexRec.place(x=10, y=10)
